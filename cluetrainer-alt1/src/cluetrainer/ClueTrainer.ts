...
export class SettingsManagement {
  ...
  observable_settings: Observable.Simple<Settings.Settings> = observe<Settings.Settings>(null).equality(FakeLodash.isEqual)

  observable = {
    scans: this.observable_settings.map(s => s?.solving?.scans).structuralEquality()
  }

  active_teleport_customization: Observable<ActiveTeleportCustomization> = observe(null).equality(FakeLodash.isEqual)
