 ...
 *
 * Logic specific to different types of clues and puzzles is outsourced in subclasses of {@link ClueSolvingSubBehaviour}.
 */
export default class ClueSolvingBehaviour extends Behaviour {
  public map_layer: ClueSolvingMapLayer

  // Active clue step and step history
  private state: ClueSolving.ClueState = null
  private history: ClueSolving.ClueState[] = []
  ...
  private activateSubBehaviour(behaviour: ClueSolvingSubBehaviour) {
    if (behaviour) {
  ...
  private pushState(state: ClueSolving.ClueState["step"], read_result: ClueReader.Result | undefined): ClueSolving.ClueState {
    const now = Date.now()
    ...
    log().log(`Created state ${this.state.state_id} (${ClueSolving.ClueState.title(this.state)})`, "Solving")
  ...
  setSolutionArea(state: ClueSolving.ClueState, area: TileArea): void {
    if (!state) return
   ...
   */
  reset(state: ClueSolving.ClueState): boolean {
