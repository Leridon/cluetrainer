...
import {Clues, ClueTier, ClueType} from "../../../../lib/cluetrainer-core/src/model/Clues";
...
import {KnotSolving} from "../../cluesolving/subbehaviours/KnotSolving";
import {LockboxSolving} from "../../cluesolving/subbehaviours/LockboxSolving";
import {TowersSolving} from "../../cluesolving/subbehaviours/TowersSolving";
import {ScanSolving} from "../../cluesolving/subbehaviours/scans/ScanSolving";
import {Alt1Color} from "../../../lib/alt1/Alt1Color";
import {ScanControlPrototype} from "../../cluesolving/subbehaviours/scans/ScanInputBehaviour";
...
import {AugmentedMethod} from "../../../../lib/cluetrainer-core/src/model/MethodPack";
import cls = C.cls;
  ...
  render() {
    ...
    for (let color of PotaColor.values) {
      ...
      this.layout.row(new Checkbox(
          hboxl(inlineimg(PotaColor.iconUrl(color)), `${FakeLodash.capitalize(color)} Passage of the Abyss`)
        )
          .setValue(!!definition)
          .onCommit(v => {
            if (v) {
    ...
    const pota_checks = PotaColor.values.map(color =>
      new Checkbox(
        hboxl(inlineimg(PotaColor.iconUrl(color)), FakeLodash.capitalize(color))
      )
  ...
  render() {
    ...
    this.layout.namedSetting("Max Zoom",
      hbox(
        ...
        NislIcon.reset2().withClick(() => {
          this.value.global_max_zoom = ClueSolving.Settings.GeneralSettings.unwrap().shape.global_max_zoom.parse(undefined)
          this.render()
    ...
    this.layout.namedSetting("Minimum Area",
      hbox(
        ...
        NislIcon.reset2().withClick(() => {
          this.value.minimum_view_size = ClueSolving.Settings.GeneralSettings.unwrap().shape.minimum_view_size.parse(undefined)
          this.render()
        }).tooltip("Reset to default"),
      ),
...
class CompassSettingsEdit extends AbstractSettingsEdit<CompassSolving.Settings> {
  ...
  render() {
    ...
    for (const compass of clue_data.compass) {
      ...
      this.layout.named(hboxl(inlineimg(ClueType.meta(compass.tier).icon_url), FakeLodash.capitalize(compass.tier)), preset_selector)
    }
      ...
      this.layout.named("Tier", new DropdownSelection<Clues.Compass>({
          type_class: {
            toHTML: (v: Clues.Compass) => {
              return hboxl(inlineimg(ClueType.meta(v.tier).icon_url), FakeLodash.capitalize(v.tier))
            }
          }
        }, clue_data.compass)
          .setValue(clue)
...
export class SettingsModal extends FormModal<{
  ...
  private hasUnsavedChanges(): boolean {
    return !FakeLodash.isEqual(deps().app.settings.settings, this.edit.value)
  }
