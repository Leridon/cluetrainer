...
import {ScanProfile} from "../../../cluetheory/scans/ScanProfile";
import ScanEquivalenceClassOptions = ScanProfile.ScanEquivalenceClassOptions;
...
class EquivalenceClassHandling extends Behaviour {
  ...
  private render_equivalence_classes(ecs: ScanEquivalenceClasses): leaflet.FeatureGroup {
    ...
    ecs.equivalence_classes.map((c) => {
      ...
      areaToPolygon(
        ecs.raster,
        (p: ScanProfileEquivalenceClass) => p.id == c.id,
        c.area[0]
      ).setStyle({
        color: "black",
        opacity: 1,
