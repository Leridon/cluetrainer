...
export default class MethodPackWidget extends Widget {
  constructor(public pack: MethodPack,
              private manager: MethodPackManager,
              customization: {
                buttons?: boolean,
                collapsible?: boolean
              },
              private open_handler: (_: MethodPack) => void = undefined
  ) {
    super();
    ...
    const header = C.hbox(
        ...
        spacer(),
        C.div(span(FakeLodash.capitalize(pack.type))
          .addClass("ctr-pack-widget-header-type")
          .addClass(MethodPack.isEditableDefault(pack)
  ...
  private openContextMenu(event: JQuery.MouseEventBase) {
    ...
    if (MethodPack.isEditable(pack) || pack.type == "imported") {
      menu.children.push({
        type: "basic",
        text: "Delete",
