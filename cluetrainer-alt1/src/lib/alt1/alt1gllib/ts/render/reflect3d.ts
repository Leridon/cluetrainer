...
import { RenderInvocation, VertexArray } from "../util/alt1gltypes";
...
export function parseGameState(renders: RenderInvocation[], cache: BufferCache) {
	return renders.map(q => cache.getMeshData(q)).filter(q => !!q);
}

export function parseGameStateStructure(renders: RenderInvocation[], cache: BufferCache) {
...
export function getGameState(renders: RenderInvocation[], cache: BufferCache) {
