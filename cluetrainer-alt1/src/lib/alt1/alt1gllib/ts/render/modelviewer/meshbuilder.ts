...
import { Alt1GlClient, RecordRenderOptions, RenderInvocation, TextureSnapshot } from "../../util/alt1gltypes";
...
export async function captureRenderCallsRaw(api: Alt1GlClient, mode: CaptureMode, captopts: RecordRenderOptions = {}) {
    //var json = patchrs.programInfo(-1, (mode == "ava" ? "avatar" : "all"))(5000)!;
    //cast to [] so ignore type issues with symbol disposable 
    var json: RenderInvocation[] = await api.recordRenderCalls(captopts);
...
export function convertCallsToRenderfuncs(calls: RenderInvocation[]) {
    // return calls.map(j => {
...
function materialSource<T extends MaterialSet<any>>(texpacker: T, meshcallback: (p: Mesh<T extends MaterialSet<infer Q> ? Q : any>) => any) {
    ...
    var getMaterial = (renderfunc: RenderFunc, texture: { atlas: TextureSnapshot, atlasmeta: TextureSnapshot }, idx: number, idy: number) => {
...
export function convertUniformAnim(renders: RenderInvocation[], program: ProgramMeta) {
...
export async function recordGltfAnimation(api: Alt1GlClient, duration: number, filter: CaptureMode | number, opts?: { captmode?: "ava" | "all" | "ui3d", resampletime?: number, mode?: "all" | "extract", animdelay?: number, captureopts: RecordRenderOptions }) {
    var invokes = await captureRenderCallsRaw(api, typeof filter == "string" ? filter : "all", { framebufferTexture: (typeof filter == "number" ? filter : undefined), ...opts?.captureopts ?? {} });
    ...
    return convertRendersToGLTF(renderfn, async (renderfn) => {
        ...
        var cbs = renderfn.map(r => ({
            orig: r,
            prom: api.recordRenderCalls({ maxframes: 1000, vertexObjectId: r.raw.vertexObjectId, programId: r.raw.program.programId, timeout: duration })
        }));
...
export async function captureGltfModel(api: Alt1GlClient, mode: CaptureMode = "ava") {
    var invokes = await captureRenderCallsRaw(api, mode);
