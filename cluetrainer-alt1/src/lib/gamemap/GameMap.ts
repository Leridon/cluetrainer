...
export class GameMap extends leaflet.Map {
  ...
  private updateView(): this {
    ...
    this.viewport.set({
      rect: TileRectangle.lift(Rectangle.extend(Rectangle.from(LeafletUtils.latLongToVector2(bounds.getNorthEast()), LeafletUtils.latLongToVector2(bounds.getSouthWest())), 1), this.floor.value()),
      zoom: this.getZoom()
    })
  ...
  public fitView(view: TileRectangle, options: FitBoundsOptions = {}): this {
    ...
    const finalbounds = LeafletUtils.convert_bounds(LeafletUtils.boundsFromRectangle(view)).pad(0.1)
  ...
  getClientPos(coordinates: Vector2): Vector2 {
    return Vector2.add(this.latLngToContainerPoint(LeafletUtils.latLongFromVector2(coordinates)), {
