import * as patchrs from "../util/alt1gltypes";
...
import { RenderInvocation } from "../util/alt1gltypes";
import { getProgramFlag, getVertexFlag } from "../util/patchrs_napi";
...
export class BufferCache {
	...
	obsoleteMeshFlag = getVertexFlag();
	knownMeshFlag = getVertexFlag();

	unknownProgramFlag = getProgramFlag();
	uiProgramFlag = getProgramFlag();
	objectProgramFlag = getProgramFlag();
	highlightProgramFlag = getProgramFlag();

	api: patchrs.Alt1GlClient | null;
	constructor(api: patchrs.Alt1GlClient | null) {
		this.api = api;
	}
	...
	clone() {
		let r = new BufferCache(this.api);
		//can grow
...
export class FilteredGameState extends TypedEmitter<{ update: void }> {
	...
	async fullCapture(sparse = false) {
		if (!this.cachedBuffers.api) { throw new Error("no api"); }
		let renders = await this.cachedBuffers.api.recordRenderCalls({
	...
	track(sparse = false) {
		...
		if (!this.npcsstream) {
			...
			let raw = this.cachedBuffers.api.streamRenderCalls(settings, this.updateRenders);
...
function updateCharRefPos(chr: CharacterRef, mesh: WorldMesh, render: RenderInvocation) {
