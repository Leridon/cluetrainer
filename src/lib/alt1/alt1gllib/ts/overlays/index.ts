...
import { Alt1GlClient, GlUniformArgument } from "../util/alt1gltypes";
import "./tilemarkers";
...
export async function findFloorRender(api: Alt1GlClient) {
    let renders = await api.recordRenderCalls();
...
export class UniformSnapshotBuilder<T extends { [name: string]: keyof typeof uniformTypes }> {
    ...
    constructor(init: T) {
        ...
        for (let [name, type] of Object.entries(init)) {
            ...
            let entry: GlUniformArgument = { name, length: 1, type: t.type, snapshotOffset: offset, snapshotSize: t.size };
...
export async function test4(api: Alt1GlClient) {
    let floor = await findFloorRender(api);
    if (!floor) {
        console.log("floor program not found");
        return;
    }

    let prog = api.createProgram(vertshader, fragshader, [
    ...
    let vertex = api.createVertexArray(indexbuffer, [
    ...
    let overlay = api.beginOverlay({ programId: floor.program.programId }, prog, vertex, {
        uniformSources: [
