...
import { Alt1GlClient, GlOverlay, GlProgram, OverlayUniformSource, RenderInvocation } from "../util/alt1gltypes";
...
class FloorOverlayChunk {
    ...
    async load() {
        ...
        if (this.settings.collision) {
            ...
            let vertex = this.api.createVertexArray(mesh.index, [
            ...
            let { program, uniforms, uniformsources } = floorOverlayProgram(this.api);
            ...
            this.overlayhandles.push(this.api.beginOverlay({ skipProgramMask: wrongprogmask, vertexObjectId: this.targetVertexObject }, program, vertex, {
    ...
    constructor(api: Alt1GlClient, render: RenderInvocation, settings: OverlaySettings) {
        this.api = api;
...
export function floorTracker(api: Alt1GlClient, settings: OverlaySettings) {
    let knownProgs = new WeakMap<GlProgram, {}>();
    let chunks = new Map<number, FloorOverlayChunk>();
    let stopped = false;

    let stream = api.streamRenderCalls({
        features: ["uniforms"],
        framecooldown: 2000,
        ...
        for (let render of renders) {
            ...
            let chunk = getOrInsert(chunks, render.vertexObjectId, () => new FloorOverlayChunk(api, render, settings));
...
function floorOverlayProgram(api: Alt1GlClient) {
    ...
    let uniformsources: OverlayUniformSource[] = [
    ...
    let program = api.createProgram(vertshadermouse, fragshaderflatnormals, [
