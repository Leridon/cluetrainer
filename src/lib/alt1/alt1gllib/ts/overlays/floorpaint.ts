...
import { Alt1GlClient, OverlayUniformSource, RenderFilter } from "../util/alt1gltypes";
...
async function findBestFloorRender(api: Alt1GlClient) {
    let allrenders = await api.recordRenderCalls({ features: ["uniforms", "vertexarray"] });
...
export async function setFloorOverlay(api: Alt1GlClient, img: ImageData) {
    let render = await findBestFloorRender(api);
    ...
    let { program, uniforms, uniformsources } = floorOverlayProgram(api, posloc);
    uniforms.mappings.uTexture.write([0]);

    let tex = api.createTexture(img);

    let target: RenderFilter = {
    ...
    let overlayhandle = api.beginOverlay(target, program, vertex, {
...
function floorOverlayProgram(api: Alt1GlClient, poslocation: number) {
    ...
    let program = api.createProgram(vertexshader.replaceAll("{{poslocation}}", "" + poslocation), fragmentshader, [
