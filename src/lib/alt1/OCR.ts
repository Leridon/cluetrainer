...
export namespace OCR {
   ...
   */
  export function canblend(rm: number, gm: number, bm: number, r1: number, g1: number, b1: number, p: number): number {
    const m = Math.min(50, p / (1 - p));
    const r = rm + (rm - r1) * m;
    const g = gm + (gm - g1) * m;
    const b = bm + (bm - b1) * m;
  ...
  export function readLine(buffer: ImageData, font: FontDefinition, colors: ColorTriplet | ColorTriplet[], x: number, y: number, forward: boolean, backward = false) {
    ...
    let addfrag = (forward: boolean) => {
      ...
      if (forward) { fragments.push(frag); } else { fragments.unshift(frag); }
      fragtext = "";
      fragstartdx = dx;
    ...
    for (var dirforward of [true, false]) {
      ...
      while (true) {
        ...
        if (col == null || chr == null) {
          ...
          if (dirforward) { x2 = x + dx - font.spacewidth; } else { x1 = x + dx + font.spacewidth; }
          break;
        } else {
          ...
          if (dirforward) { fragtext += spaces + chr.chr; } else { fragtext = chr.chr + spaces + fragtext; }
  ...
  export function readChar(buffer: ImageData, font: FontDefinition, col: ColorTriplet, x: number, y: number, backwards: boolean, allowSecondary?: boolean): ReadCharInfo | null {
    ...
    const candidate_scores: { score: number, sizescore: number, chr: Charinfo }[] = [];

    charloop: for (const chrobj of font.chars) {
      if (chrobj.secondary && !allowSecondary) continue

      let score = 0

      const chrx = (backwards ? x - chrobj.width : x);

      if (shadow) {
        for (let a = 0; a < chrobj.pixels.length; a += 4) {
          const i = (chrx + chrobj.pixels[a]) * 4 + (y + chrobj.pixels[a + 1]) * buffer.width * 4;

          const lum = chrobj.pixels[a + 3] / 255;
          score += canblend(buffer.data[i], buffer.data[i + 1], buffer.data[i + 2], col[0] * lum, col[1] * lum, col[2] * lum, chrobj.pixels[a + 2] / 255);

          if (score > DETECTION_THRESHOLD) continue charloop // Early exit if score has surpassed the threshold already
        }
      } else {
        for (let a = 0; a < chrobj.pixels.length; a += 3) {
          const i = (chrx + chrobj.pixels[a]) * 4 + (y + chrobj.pixels[a + 1]) * buffer.width * 4;

          score += canblend(buffer.data[i], buffer.data[i + 1], buffer.data[i + 2], col[0], col[1], col[2], chrobj.pixels[a + 2] / 255);
   ...
   */
  export function generateFont(unblended: ImageData, chars: string, seconds: string, bonusses: {
    [char: string]: number
