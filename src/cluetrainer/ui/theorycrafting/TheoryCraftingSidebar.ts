...
export default class TheoryCraftingSidebar extends MapSideBar {
  ...
  private opened_pack = observe<MethodPack>(null)
    ...
    if (focus) {
      ...
      props.row(btnrow(
        new LightButton("Import", "rectangle")
          .onClick(async () => {
            const imported = await new ImportStringModal<MethodPack>(imp({expected_type: "method-pack", expected_version: 1})).do()
      ...
      packs.forEach(p => {
        new MethodPackWidget(p, MethodPackManager.instance(), {buttons: true, collapsible: true}, pack => this.opened_pack.set(pack)).appendTo(this.body)
      })
    }
  }
