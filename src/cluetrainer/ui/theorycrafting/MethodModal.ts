...
class MethodPackSelector extends AbstractEditWidget<MethodPack> {
  ...
  async render() {
    ...
    this.selector = new DropdownSelection<{
      ...
      .setItems(async () => {
        return [
          ...(await MethodPackManager.instance().all()).filter(p => MethodPack.isEditable(p)).map(p => ({pack: p})),
...
export class NewMethodModal extends FormModal<{
  ...
  pack_selector: MethodPackSelector
  edit: MethodMetaEdit
  ...
  render() {
    ...
    new Properties().appendTo(this.body)
      .named("Pack", this.pack_selector = new MethodPackSelector()
