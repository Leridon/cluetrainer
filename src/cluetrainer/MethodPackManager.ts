...
import {ClueSpotIndex} from "./model/clues/ClueIndex";
...
export class MethodPackManager {
  ...
  public readonly local_pack_store = KeyValueStore.instance().variable<MethodPack[]>("data/local_methods")

  private default_packs: MethodPack[]
  private local_packs: MethodPack[] = []

  pack_set_changed: Ewent.Real<MethodPack[]> = ewent()
  ...
  async getPack(local_id: string): Promise<MethodPack> {
    return (await this.all()).find(p => p.local_id == local_id)
  }

  async deletePack(pack: MethodPack, save: boolean = true) {
  ...
  async updatePack(pack: MethodPack, f: (_: MethodPack) => any): Promise<MethodPack> {
  ...
  async replacePack(existing: MethodPack, updated: MethodPack) {
