...
export class WikiPageCompassAngleUncertainty extends WikiPage {
  render(): void {
    ...
    this.paragraph("Unsurprisingly, angle uncertainty refers to the uncertainty about the angle detected by Clue Trainer when reading the compass arrow. It affects the angle of the triangulation beam. Sources of angle uncertainty can be split into calibration uncertainty and resolution uncertainty.")

    this.header("Resolution Uncertainty", "left", 1)

    this.paragraph("Resolution inaccuracy refers to the inherent inaccuracy of the compass arrow. The game's renderer can only display around 2,000 distinct angles, so there is a natural uncertainty of ±0.09° on average. This is the lower limit for angle uncertainty.")
    ...
    this.paragraph("The rendering of the compass arrow changes depending on whether antialising is activated in the game's graphics settings (Specifically MSAA). Antialiasing changes the rendering in inconsistent ways, causing the compass reader to produce slightly different results on different devices. This is why the compass reader is mostly calibrated with ", bold("antialiasing off"), ". When antialising is on, the compass reader adds a flat uncertainty of ±0.5° to be safe. To get the best results, especially when you use a static teleport for triangulation, ", bold("you should turn antialiasing off"), ".")
  }
}