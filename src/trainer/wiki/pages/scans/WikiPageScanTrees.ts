...
export class WikiPageScanTrees extends WikiPage {
  render() {
    ...
    this.paragraph("Clue Trainer marks decision spots in green on the map. They may cover multiple tiles due to non-static teleports or larger ", text_link("equivalence classes", () => ClueTrainerWiki.open("scanequivalenceclasses")), ".")
    ...
    this.row(new List()
      ...
      .item("Using the ", text_link("interactive scan tree control overlay", () => ClueTrainerWiki.open("scantreecontroloverlay")), ".")
    )
    ...
    this.seeAlso([
      {action: () => ClueTrainerWiki.open("scantreeediting"), name: "Creating custom scan trees"}
    ])
  }
}