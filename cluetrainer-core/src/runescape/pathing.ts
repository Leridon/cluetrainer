...
export namespace Path {
            ...
            if (state.position.tile && !TileCoordinates.eq(state.position.tile, step.from)) {
              ...
              if (!res || !TileCoordinates.eq(step.to, res.tile))
                if (step.ability == "escape" || step.ability == "surge")
                  augmented.issues.push({level: "error", message: `${FakeLodash.capitalize(step.ability)} target does not match where it would end up!`})
                else
                  augmented.issues.push({level: "error", message: `${FakeLodash.capitalize(step.ability)} target can't be reached!`})
            }
          }
          ...
          if (next_available_charge.remaining_cooldown > 0) {
            ...
            if (next_available_charge.charge.cooldown_reason == "antispam") {
              ...
              if (state.cooldowns[step.ability].length > 1) {
                ...
                augmented.issues.push({level: severity, message: `${FakeLodash.capitalize(step.ability)} still on cooldown for ${next_available_charge.remaining_cooldown} ticks!`})
              }
            }
  ...
  export namespace Step {
    ...
    export function name(step: Step): string {
      switch (step.type) {
        case "ability":
          return FakeLodash.capitalize(step.ability)
        default:
          return FakeLodash.capitalize(step.type.toUpperCase())
      }
    }
  }
