...
export namespace CelticKnots {
  ...
  export namespace PuzzleShape {
    import natural_order = Order.natural_order;

    export function isEqual(a: PuzzleShape, b: PuzzleShape) {
      return FakeLodash.isEqual(a.snake_lengths, b.snake_lengths) && a.locks.length == b.locks.length
    }
  ...
  export namespace PuzzleState {
    export function generate(shape: PuzzleShape): PuzzleState {
      const state: PuzzleState = {
        shape: shape,
        snakes: shape.snake_lengths.map(s => new Array(s).fill(0).map(() => ({value: FakeLodash.random(20), type: "is"})))
      }
    ...
    export function shuffle(state: PuzzleState): PuzzleState {
      return {
        ...
        snakes: state.snakes.map(snake => {
          const o = FakeLodash.random(snake.length)
  ...
  export function unify(a: PuzzleState, b: PuzzleState): PuzzleState | null {
    if (!a || !b) return null
    ...
    function unifyElement(a: Element, b: Element): Element | null {
  ...
  export function solve(state: PuzzleState): Solution | undefined {
