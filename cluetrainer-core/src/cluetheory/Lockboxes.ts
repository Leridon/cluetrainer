...
export namespace Lockboxes {
  ...
  export namespace MoveMap {
    ...
    let all_quiet_patterns: Lazy<MoveMap[]> = lazy(() => {
    ...
    export function getEquivalents(map: MoveMap): MoveMap[] {
      return all_quiet_patterns.get().map(quiet => chain(map, quiet))
    }
  ...
  export namespace State {
    ...
    export function generate(): State {
      return {
        tile_rows: SOLVED.tile_rows.map(r => r.map(() => FakeLodash.random(0, 2) as Tile))
      }
    }
